...
/*
 * Berend: Linked list functionality
 */
...
void list_init(List* list) {
  list->head = NULL;
  list->tail = NULL;
  list->head_ptr = &list->head;
}

void list_add_node(List* list, int val) {
  List_node* new_node = malloc(sizeof(List_node));
  new_node->val = val;
  new_node->next = NULL;
  
  (*list->head_ptr) = new_node;
  list->head_ptr = &new_node->next;
  list->tail = new_node;
  list->size++;
}

void list_print_values(List* list) {
...
static List testing_list;
static List_node* false_node;
...
{
  if (setup_mode == 1) {
    list_init(&testing_list);
    false_node = malloc(sizeof(List_node));
    false_node->val = 666;
    false_node->next = malloc(sizeof(List_node));
    false_node->next->val = 667;
    setup_mode = 0;
  } else {
    list_add_node(&testing_list, open_counter);
    list_print_values(&testing_list);
  }
