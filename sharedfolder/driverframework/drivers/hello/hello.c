...
char * weird_pointer_thingy = NULL;
 ...
 */
static int hello_open(devminor_t minor, int access, endpoint_t
user_endpt);
...
static ssize_t hello_read(devminor_t UNUSED(minor), u64_t position, endpoint_t endpt, cp_grant_id_t grant, size_t size, int UNUSED(flags), cdev_id_t UNUSED(id)) {
    u64_t dev_size;
    char *ptr;
    int ret;
    char *buf = HELLO_MESSAGE;
    printf("Invalidating weird_pointer_thingy\n");
    free(weird_pointer_thingy);
...
static void sef_local_startup() {
    ...
    weird_pointer_thingy = malloc(100);
    weird_pointer_thingy = "AAAAAAAAAAAAAAAAAAAA";
